...
<template>
  ...
  <div class="hero bg-base-200 m-0 p-5 rounded-lg shadow-lg w-full">
    <div>
      <h3 class="text-6xl font-bold mb-5">Laporan pemakaian Barang</h3>
      ...
      <div class="flex justify-between w-full">
        <div class="badge badge-outline mb-2 text-left">
          {{ laporan?.tanggal ?? "Tangaal tidak ditemukan" }}
        </div>
        <div class="badge badge-outline mb-2 text-left">
          Jumlah pesanan :
          {{ laporan?.jumlah_pesanan ?? "jumlah tidak di temikan" }}
        </div>
...
export default {
  ...
  methods: {
    ...
    downloadPDF() {
      const doc = new jsPDF();
      const { description, tanggal, jumlah_pesanan, barang, user } =
        this.laporan;
      ...
      doc.text(`Nama User: ${user?.name ?? "Tidak ditemukan"}`, 10, 45); // Nama user di kiri
      ...
      doc.text(` Jumlah pesanan: ${jumlah_pesanan}`, 10, 60);
      doc.text(` ${description}`, 30, 80);

      // Mengunduh file PDF
      doc.save("Pesanan_barang.pdf");
    },
  },
};
</script>
